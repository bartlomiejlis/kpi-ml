...
def train_and_save_model(order):
    arma_model = SARIMAX(y, order=order)
    ...
    if rmse <= 0.03:
        model_name = 'ARMA_model_with_score_' + str(rmse)
        joblib.dump(arma_model, model_name)
...
def create_plot(model_name):
    ...
    plt.plot(y_pred_out, color='green', label='Prognoza ARMA')
    plt.legend()
    plt.ylabel('Procent obrotu')
    plt.xlabel('Okres')
    plt.xticks(rotation=45)
    plt.subplots_adjust(bottom=0.2)
    plt.title('Prognoza procentu obrotu wg modelu ARMA')
    plt.show()
